...
class Args:
    def __init__(self, cfg_path, gpu_id=1, model_type='vicuna', options=[]):
...
for user_message in user_message_list:
    file_path = f"./results/prompt_{user_message}.json"

    data = {}
    for file_n in file_names:
        key = file_n[:-4]
        tmp_filtered_movies = filtered_movies[file_n[:-4]]
        # print(user_message, file_n)
        result = perform_inference(os.path.join(directory, file_n), input_type, user_message)
        # print(result)
